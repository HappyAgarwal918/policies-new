  ...
  <div class="container">
    <div class="panel-group">
      <div class="panel panel-primary">
        <form action="<?php if(isset(auth()->user()->admin) && (auth()->user()->admin == 1 || auth()->user()->admin == 2) || $policy_data->payment_status == 'succeeded'){ echo 'complete?form-id='.$id; } elseif($policy_data->premium == 1 || $policy_data->company == 1 || $policy_data->specialty == 'fitness' || $policy_data->specialty == 'financial' || $policy_data->specialty == 'investments' || $policy_data->specialty == 'medical' || $policy_data->specialty == 'legal' || $policy_data->accept == 1 || $policy_data->affiliate_links_amazon == 1 || $policy_data->ads == 1){ echo 'billing?form-id='.$id; } else{ echo "complete?form-id='.$id"; } ?>" method="post" id="">
          @csrf
          <div class="panel-body">
            ...
            <div class="form-group form_part">
              <label class="control-label col-sm-12 pb-2" for=""><?php if($policy_data->platforms == 'Website URL' || $policy_data->platforms == 'Mobile App Name') { echo $policy_data -> platforms; } else { echo $policy_data -> platforms_other; } ?></label>
              <div class="col-sm-6">
                <strong>
                  <?php if ($policy_data->platforms = 'Mobile App Name'){ echo $policy_data -> mobile_name; }
                        ...
                        if ($policy_data->platforms_other = 'ebook Name'){ echo $policy_data -> ebook_name; }
                        if ($policy_data->platforms_other = 'Landing Page') {echo $policy_data -> landing_url;}
                        if ($policy_data->platforms_other = 'Course Name') { echo $policy_data -> course_name;}
                        if ($policy_data->platforms_other = 'Podcast'){ echo $policy_data -> podcast_name; }
                        if ($policy_data->platforms_other = 'Video Name'){ echo $policy_data -> video_name; } ?>
                </strong>
              </div>
            </div>
            @auth
            <div class="form-group form_part">
              ...
              <span class="help">You will receive your disclaimer policy to this email address</span>
              @error('email')
            ...
            @endguest
            @if($policy_data->premium == 1 || $policy_data->company == 1 || $policy_data->specialty == 'fitness' || $policy_data->specialty == 'financial' || $policy_data->specialty == 'investments' || $policy_data->specialty == 'medical' || $policy_data->specialty == 'legal' || $policy_data->accept == 1 || $policy_data->affiliate_links_amazon == 1 || $policy_data->ads == 1)
            <input type="hidden" name="url" value="<?php echo 'disclaimer-premium-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'billing?form-id='.$id ?>" />
            <input type="hidden" name="type" value="Premium" />
            @else
            <input type="hidden" name="url" value="<?php echo 'disclaimer-basic-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'disclaimer-basic-policy?form-id='.$id ?>" />
          ...
          <div>
              <button type="submit" class="ybtn ybtn-accent-color">Create Disclaimer Policy</button>
          </div>
        </form>
      </div>
    </div>
