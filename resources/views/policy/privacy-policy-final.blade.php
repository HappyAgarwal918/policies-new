  ...
  <div class="container">
    <div class="panel-group">
      <div class="panel panel-primary">
        <form action="<?php if(isset(auth()->user()->admin) && (auth()->user()->admin == 1 || auth()->user()->admin == 2) || $policy_data->payment_status == 'succeeded'){ echo 'complete?form-id='.$id; } elseif($policy_data->premium == 1 || $policy_data->company == 1 || $policy_data->register_social == 1 || $policy_data->teens == 1 || $policy_data->sell == 1 || $policy_data->affiliate_links == 1 || $policy_data->ads == 1 || $policy_data->remarketing == 1 || $policy_data->info == 'identity' || $policy_data->info == 'payment' || $policy_data->purpose == 'sell_services' || $policy_data->purpose == 'deliver_services' || $policy_data->purpose == 'improve_services' || $policy_data->purpose == 'send_promo' || $policy_data->purpose == 'share_testimonials' || $policy_data->purpose == 'show_ads' || $policy_data->sell_a == 1 || $policy_data->merger == 1){ echo 'billing?form-id='.$id; } else{ echo "complete?form-id='.$id"; } ?>" method="post" id="">
          @csrf
          <div class="panel-body">
            ...
            <div class="form-group form_part">
              ...
              <span class="help">You will receive your Privacy policy to this email address</span>
              @error('email')
            ...
            @endguest
            @if($policy_data->premium == 1 || $policy_data->company == 1 || $policy_data->register_social == 1 || $policy_data->teens == 1 || $policy_data->sell == 1 || $policy_data->affiliate_links == 1 || $policy_data->ads == 1 || $policy_data->remarketing == 1 || $policy_data->info == 'identity' || $policy_data->info == 'payment' || $policy_data->purpose == 'sell_services' || $policy_data->purpose == 'deliver_services' || $policy_data->purpose == 'improve_services' || $policy_data->purpose == 'send_promo' || $policy_data->purpose == 'share_testimonials' || $policy_data->purpose == 'show_ads' || $policy_data->sell_a == 1 || $policy_data->merger == 1)
            <input type="hidden" name="url" value="<?php echo 'privacy-premium-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'billing?form-id='.$id ?>" />
            <input type="hidden" name="type" value="Premium" />
            @else
            <input type="hidden" name="url" value="<?php echo 'privacy-basic-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'privacy-basic-policy?form-id='.$id ?>" />
          ...
          <div>
              <button type="submit" class="ybtn ybtn-accent-color">Create Privacy Policy</button>
          </div>
        </form>
      </div>
    </div>
