  ...
  <div class="container">
    <div class="panel-group">
      <div class="panel panel-primary">
        <form action="<?php if(isset(auth()->user()->admin) && (auth()->user()->admin == 1 || auth()->user()->admin == 2) || $policy_data->payment_status == 'succeeded'){ echo 'complete?form-id='.$id; } elseif($policy_data->premium == 1 || $policy_data->company == 1 || $policy_data->sell == 1 || $policy_data->ads == 1 || $policy_data->misc_clauses == 'assignment' || $policy_data->misc_clauses == 'warranty'){ echo 'billing?form-id='.$id; } else{ echo "complete?form-id='.$id"; } ?>" method="post" id="">
          @csrf
          <div class="panel-body">
            ...
            <div class="form-group form_part">
              ...
              <span class="help">You will receive your Terms and Conditions policy to this email address</span>
              @error('email')
            ...
            @endguest
            @if($policy_data->premium == 1 || $policy_data->company == 1 || $policy_data->sell == 1 || $policy_data->ads == 1 || $policy_data->misc_clauses == 'assignment' || $policy_data->misc_clauses == 'warranty')
            <input type="hidden" name="url" value="<?php echo 'terms-and-conditions-premium-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'billing?form-id='.$id ?>" />
            <input type="hidden" name="type" value="Premium" />
            @else
            <input type="hidden" name="url" value="<?php echo 'terms-and-conditions-basic-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'terms-and-conditions-basic-policy?form-id='.$id ?>" />
          ...
          <div>
              <button type="submit" class="ybtn ybtn-accent-color">Create Terms and Conditions Policy</button>
          </div>
        </form>
      </div>
    </div>
