  ...
  <div class="container">
    <div class="panel-group">
      <div class="panel panel-primary">
        <form action="<?php if(isset(auth()->user()->admin) && (auth()->user()->admin == 1 || auth()->user()->admin == 2) || $policy_data->payment_status == 'succeeded'){ echo 'complete?form-id='.$id; } elseif($policy_data->premium == 1 || $policy_data->sell == 'tangible' || $policy_data->money_back == 0){ echo 'billing?form-id='.$id; } else{ echo "complete?form-id='.$id"; } ?>" method="post" id="">
          @csrf
          <div class="panel-body">
            ...
            <div class="form-group form_part">
              ...
              <span class="help">You will receive your Refund policy to this email address</span>
              @error('email')
            ...
            @endguest
            @if($policy_data->premium == 1 || $policy_data->sell == 'tangible' || $policy_data->money_back == 0)
            <input type="hidden" name="url" value="<?php echo 'refund-premium-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'billing?form-id='.$id ?>" />
            <input type="hidden" name="type" value="Premium" />
            @else
            <input type="hidden" name="url" value="<?php echo 'refund-basic-policy?form-id='.$id ?>" />
            <input type="hidden" name="link" value="<?php echo 'refund-basic-policy?form-id='.$id ?>" />
          ...
          <div>
              <button type="submit" class="ybtn ybtn-accent-color">Create Refund Policy</button>
          </div>
        </form>
      </div>
    </div>
